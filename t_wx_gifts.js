...
[task_local]
# 生日礼包
7 7 7 7 7  t_wx_gifts.js, tag=生日礼包, enabled=true
 */
const $ = new Env('生日礼包');
...
$.activityId = process.env.T_WX_GIFTS_ACTIVITY_ID ? process.env.T_WX_GIFTS_ACTIVITY_ID : "";
$.activityUrl = `https://cjhy-isv.isvjcloud.com/mc/wxMcLevelAndBirthGifts/activity?activityId=${$.activityId}`
$.openCard = false
$.exportActivityIds = ""
$.friendUuid = ""
$.friendUuids = []
...
async function jdmodule() {
    ...
    await takePostRequest("getMemberLevel")
    ...
    if ($.activityType == 103) {
        ...
        if ($.isFirst) {
            ...
            console.log(`已经领取过！`)
            $.message += `京东账号 ${$.nickName || $.UserName} 已经领取过礼包`
        }
    } else {
        await takePostRequest("sendLevelGifts")
    }
}
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLog':
            ...
            let pageurl = `${$.activityUrl}&venderId=${$.venderId}&sid=&un_area=13_1007_4909_59742`
            ...
            break;
        case 'getMemberLevel':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/getMemberLevel`;
            body = `pin=${encodeURIComponent(encodeURIComponent($.Pin))}&venderId=${$.venderId}`;
            break;
        case 'activityContent':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/activityContent`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent(encodeURIComponent($.Pin))}&level=${$.level}`
            break;
        case 'getBirthInfo':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/getBirthInfo`;
            body = `pin=${encodeURIComponent(encodeURIComponent($.Pin))}&venderId=${$.venderId}`;
            break;
        case 'saveBirthDay':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/saveBirthDay`;
            body = `pin=${encodeURIComponent(encodeURIComponent($.Pin))}&venderId=${$.venderId}&birthDay=${$.today}`;
            break;
        case 'sendBirthGifts':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/sendBirthGifts`;
            body = `activityId=${$.activityId}&venderId=${$.venderId}&pin=${encodeURIComponent(encodeURIComponent($.Pin))}&level=${$.level}`
            break;
        case 'sendLevelGifts':
            url = `https://${$.domain}/mc/wxMcLevelAndBirthGifts/sendLevelGifts`;
            body = `activityId=${$.activityId}&venderId=${$.venderId}&pin=${encodeURIComponent(encodeURIComponent($.Pin))}&level=${$.level}`
            break;
        default:
            console.log(`错误${type}`);
