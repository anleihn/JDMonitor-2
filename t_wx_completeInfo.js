...
[task_local]
# 完善信息有礼
cron 7 7 7 7 *  t_wx_completeInfo.js, tag=完善信息有礼, enabled=true
 */
...
$.activityUrl = process.env.T_WX_COMPLETE_INFO_ACTIVITY_URL ? process.env.T_WX_COMPLETE_INFO_ACTIVITY_URL : "";
...
!(async () => {
    ...
    if ($.isNode()) {
        if ($.message != '') {
            await notify.sendNotify("完善信息有礼", `${$.message}\n跳转链接\n${$.activityUrl}`)
        }
    }
})()
    .catch((e) => {
...
async function jdmodule() {
    ...
    await takePostRequest("selectById")

    await takePostRequest("listDrawContent")

    await takePostRequest("accessLog")

    await takePostRequest("save")
}
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLog':
            ...
            break;
        case 'selectById':
            url = `https://${$.domain}/completeInfoActivity/selectById`;
            body = `activityId=${$.activityId}&venderId=${$.venderId}`;
            break;
        case 'listDrawContent':
            url = `https://${$.domain}/drawContent/listDrawContent`;
            body = `activityId=${$.activityId}&type=${$.activityType}`;
            break;
        case 'save':
            url = `https://${$.domain}/wx/completeInfoActivity/save`;
            body = `${$.saveBody}activityId=${$.activityId}&drawInfoId=${$.drawInfoId}&pin=${encodeURIComponent(encodeURIComponent($.Pin))}&venderId=${$.venderId}&customContent=${encodeURIComponent("[]")}&vcode=&token=${$.Token}&fromType=APP`
            break;
        default:
            console.log(`错误${type}`);
    }
            ...
            case 'selectById':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        ...
                        $.chooseMail = data.chooseMail
                        $.chooseGender = data.chooseGender
                        $.activityType = data.activityType
                        $.saveBody = ''
                        if ($.chooseName == 'y') {
                            $.name = '韩豆豆'
                            $.saveBody += `name=${encodeURIComponent($.name)}&`
                        }
                        if ($.chooseBirth == 'y') {
                            $.birth = '1995-05-06'
                            $.saveBody += `birthDay=${$.birth}&`
                        }
                        if ($.chooseGender == 'y') {
                            $.gender = '女'
                            $.saveBody += `gender=${encodeURIComponent($.gender)}&`
                        }
                        if ($.choosePhone == 'y') {
                            $.phone = '19963258745'
                            $.saveBody += `phone=${$.phone}&`
                        }
                    } else if (res.errorMessage) {
                        console.log(`${type} ${res.errorMessage || ''}`)
                    } else {
