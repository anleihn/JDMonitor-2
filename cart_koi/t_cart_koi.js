    ...
    if ($.activityIds.indexOf($.activityId) != -1) {
        ...
        for (let i = 0; i < retryNum; i++) {
            ...
            if (cookiesArr[i]) {
                ...
                await jdmodule(true);
                totalAdd = Number($.addCarts) + Number($.hasAddCartSize)
                $.message += `被助力账号${i + 1}本次加购${$.hasAddCartSize}/${totalAdd}/${$.drawCondition}/${$.totals}件商品\n`
            }
        ...
        if ($.isNode()) {
            await notify.sendNotify(`购物车锦鲤：${$.activityName}`, `${$.shopName}\n${$.message}${$.priseInfo}开奖时间：${$.drawTime}\n跳转链接: ${$.activityUrl}\n`);
            if ($.redisStatus) {
        ...
        switch (type) {
            ...
            case 'activityContent':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        ...
                        if ($.index == 2 && !$.flag) {
                            $.secondHeadHelpNum = $.totals - $.jsNum
                            $.secondOtherHelpTime = $.drawCondition - $.jsNum < 0 ? 0 : $.drawCondition - $.jsNum
                            if ($.secondHeadHelpNum != $.headHelpTimes) {
                                console.log(`发现车头助力数不对，去更换助力次数`)
                                $.headHelpTimes = $.secondHeadHelpNum
                                $.helpTimes = $.headHelpTimes
                                console.log(`车头的助力次数为${$.headHelpTimes}次`)
                            }
                            if ($.secondOtherHelpTime != $.otherHelpTime) {
                                console.log(`发现非车头账号助力数不对，去更换助力次数`)
                                $.otherHelpTime = $.secondOtherHelpTime
                                console.log(`非车头的助力次数为${$.otherHelpTime}次`)
                            }
            ...
            case 'quickAddSku':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        if (typeof res.data == 'object') {
                            $.hasAddCartSize = res.data.hasAddCartSize || 0;
