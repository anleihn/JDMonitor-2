...
$.activityUrlPrefix = 'https://lorealjdcampaign-rc.isvjcloud.com/interact/index?'
$.templateId = "202204181615yqrhylysl01"
$.activityId = "1559816237253824514"
$.acticityType = "10006"
$.shareUserId = ""
$.activityUrl = `https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=${$.acticityType}&templateId=${$.templateId}&activityId=${$.activityId}&shareUserId=`
$.Token = "";
$.openCard = false
CryptoScripts()
...
function getPostRequestJson(url, body, method = "POST") {
    ...
    if (url.indexOf($.domain) > -1) {
        headers["Referer"] = `https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=${$.acticityType}&templateId=${$.templateId}&activityId=${$.activityId}&shareUserId=${$.shareUuid}&sid=&un_area=13_1042_1048_46619`
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLogWithAD':
            ...
            let pageurl = `https://lzdz1-isv.isvjcloud.com/zst/collectCard/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
        ...
        case 'activityContent':
            url = `${domain}/zst/collectCard/activityContent`;
            body = {
            ...
            break;
        case 'login':
            url = `${domain}/app/interact/api/user-info/login`;
            body = {
                "activityId": $.activityId,
                "shareUuid": $.shareUuid || "",
                "tokenPin": $.Token,
        ...
        case 'drawContent':
            ...
            break;
        case 'getAddSkuList':
            url = `${domain}/zst/collectCard/getAddSkuList`;
            body = {
                "activityId": $.activityId,
                "pin": $.Pin,
                "uuid": $.actorUuid
        ...
        case 'getBrowseSkuList':
            url = `${domain}/zst/collectCard/getBrowseSkuList`;
            body = {
                "activityId": $.activityId,
                "pin": $.Pin,
                "uuid": $.actorUuid
            ...
            break;
        case 'saveTask':
            url = `${domain}/zst/collectCard/saveTask`;
            body = {
                "activityId": $.activityId,
                "pin": $.Pin,
                "actorUuid": $.actorUuid,
            ...
            break;
        case 'assist':
            url = `${domain}/zst/collectCard/assist`;
            body = {
                "activityId": $.activityId,
                "pin": $.Pin,
                "uuid": $.actorUuid,
            ...
            break;
        case 'exchangePrize':
            url = `${domain}/zst/collectCard/exchangePrize`;
            body = {
                "activityId": $.activityId,
                "pin": $.Pin,
                "uuid": $.actorUuid,
    ...
    let myRequest = ""
    if (isJson) {
        myRequest = getPostRequestJson(url, body, method);
        // console.log(myRequest)
    } else {
        myRequest = getPostRequest(url, body, method);
    }
    // console.log(myRequest)
    return new Promise(async resolve => {
        $.post(myRequest, (err, resp, data) => {
            ...
            case 'login':
                if (typeof res == 'object') {
                    if (res.resp_code == 0) {
                        ...
                        $.Pin = data.userPin
                        $.openCardUrl = data.openCardUrl
                        console.log(`开卡链接：${$.openCardUrl}`)
                        $.secondToken = data.token
                        console.log(`用户token：${$.secondToken}`)
                    } else if (res.errorMessage) {
                        console.log(`${type} ${res.errorMessage || ''}`)
                    } else {
                        console.log(`${type} ${data}`)
