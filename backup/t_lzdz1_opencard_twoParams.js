...
/*
三参数的非通用开卡
入口： https://lzdz1-isv.isvjcloud.com/dingzhi/XXXX/XXXX/activity?activityId=XXXX&shareUuid=XXXX
...
!(async () => {
    ...
    $.envParam = process.env.jd_opencard_three_params ? process.env.jd_opencard_three_params : "";
    $.firstParam = $.envParam.split("_")[0]
    $.secondParam = $.envParam.split("_")[1]
    $.activityId = $.envParam.split("_")[2]
    $.shareUuid = ""
    $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/${$.firstParam}/${$.secondParam}/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
    console.log(`入口:\n${$.activityUrl}`)
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            if ($.openCardPin != "") {
                if (cookie.indexOf($.openCardPin) == -1) {
                    continue
                } else {
                    console.log(`开始替换车头`)
                    temp = cookiesArr[0]
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'activityContent':
            url = `${domain}/dingzhi/${$.firstParam}/${$.secondParam}/activityContent`;
        ...
        case 'drawContent':
            ...
            break;
        case 'checkOpenCard':
            url = `${domain}/dingzhi/${$.firstParam}/${$.secondParam}/initOpenCard`;
        ...
        case 'startDraw':
            ...
            break;
        case 'followShop':
            url = `${domain}/dingzhi/${$.firstParam}/${$.secondParam}/saveTask`;
        ...
        case 'getShareRecord':
            ...
            break;
        case '抽奖':
            url = `${domain}/dingzhi/${$.firstParam}/${$.secondParam}/draw`;
