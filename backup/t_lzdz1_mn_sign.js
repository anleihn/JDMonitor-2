...
!(async () => {
    ...
    if ($.allMessage) {
        await notify.sendNotify(`蒙牛-${$.activityName}`, `${$.allMessage}跳转链接\n${$.activityUrl}`);
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLogWithAD':
            ...
            let pageurl = `${$.activityUrl}&shareUuid=${$.shareUuid}`
        ...
        case 'activityContent':
            url = `${domain}/dingzhi/mengniu/punchclock/activityContent`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${$.nickName}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`
            break;
        case 'drawContent':
