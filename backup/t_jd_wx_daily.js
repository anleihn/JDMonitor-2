...
[task_local]
# 每日抢
7 7 7 7 7  m_jd_wx_daily.js, tag=每日抢, enabled=true
 */
const $ = new Env('每日抢');
...
$.activityId = process.env.jd_daily_activityId ? process.env.jd_daily_activityId : '';
$.Token = "";
$.openCard = false
$.exportActivityIds = ""
$.friendUuid = ""
...
!(async () => {
    ...
    $.activityUrl = `https://lzkj-isv.isvjcloud.com/activity/daily/wx/indexPage?activityId=${$.activityId}`
    ...
    if ($.isNode()) {
        ...
        if ($.message != '') {
            await notify.sendNotify("每日抢", `${$.message}\n跳转链接\n${$.activityUrl}`)
        }
...
async function jdmodule() {
    ...
    console.log(`当前时间戳为${$.systime}`)
    $.startTime = moment().subtract(-1, "hours").format("YYYY-MM-DD HH:00:00")
    $.startTime_st = moment($.startTime).valueOf()
    console.log(`开抢时间为${$.startTime},对应的时间戳为${$.startTime_st}`)
    let waitTime = $.startTime_st - $.systime
    console.log(`等待${waitTime}毫秒，约${waitTime / 1000 / 60}分钟`)
    if (waitTime > 0) {
        await $.wait(waitTime)
    }
    let now = moment().format("YYYY-MM-DD HH:mm:ss")
    console.log(`当前时间为${now}，开始抢京豆`)
    await takePostRequest('grabGift');
    await $.wait(1000)
    if ($.errorMsg.indexOf(`火爆`) != -1 || $.errorMsg.indexOf(`请稍后再试`) != -1) {
        await takePostRequest('grabGift');
    }
    await $.wait(1000)
    if ($.errorMsg.indexOf(`火爆`) != -1 || $.errorMsg.indexOf(`请稍后再试`) != -1) {
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLogWithAD':
            ...
            body = `venderId=${$.venderId}&code=16&pin=${$.enPin}&activityId=${$.activityId}&pageUrl=${encodeURIComponent(pageurl1)}&subType=app&adSource=`
            break;
        case 'getUserInfo':
        ...
        case 'getSystime':
            ...
            break;
        case 'grabGift':
            url = `https://${$.domain}/activity/daily/wx/grabGift`;
        ...
        $.get(get, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    setActivityCookie(resp)
                    // $.startTime = data.split(`id="startTime"`)[1].split(`"`)[1].split(".")[0]
                    // $.startTime_st = moment($.startTime, 'YYYY-MM-DD HH:mm:ss.').valueOf()
                    // console.log(`活动开始的时间为${$.startTime}，对应的时间戳为${$.startTime_st}`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
