...
$.sevenSignScanKey = "WuXian:SevenSignScanIds"
$.conSignScanKey = "WuXian:ConSignScanIds"
if ($.isNode()) {
...
!(async () => {
    ...
    if ($.redisStatus) {
        $.recordConSign = await redisClient.get($.conSignScanKey) || ""
        $.recordSign = await redisClient.get($.sevenSignScanKey) || ""
    }
...
async function jdmodule() {
    ...
    if ($.signChange) {
        ...
        if ($.redisStatus) {
            console.log(`写入七日签到扫描Redis--->`)
            await redisClient.set($.sevenSignScanKey, $.recordSign)
            console.log(await redisClient.get($.sevenSignScanKey))
        }
    }
    if ($.conSignChange) {
        // await notify.sendNotify(`检测到连续签到变量变动`, `export RECORD_CON_SIGN=\"${$.recordConSign}\"`)
        if ($.redisStatus) {
            console.log(`写入连续签到扫描Redis--->`)
            await redisClient.set($.conSignScanKey, $.recordConSign)
            console.log(await redisClient.get($.conSignScanKey))
        }
    }
}
